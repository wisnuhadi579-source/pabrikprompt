...
export default function SceneIklanGenerator({ onBack }) {
    ...
    // --- LOGIKA PARSING PALING AMAN (FIX ERROR 'READING 0') ---
    const extractDataSafely = (data) => {
        try {
            // Cara 1: Ambil langsung dari parts[0]
            const text = data?.candidates?.[0]?.content?.parts?.[0]?.text;
            if (!text) return null;
            
            // Bersihkan dari tag ```json ... ```
    ...
    const handleGenerate = async () => {
        ...
        try {
            setLoadingMsg("Gemini 3 sedang menyusun naskah...");
            const planPrompt = `TUGAS: Buat konsep iklan ${sellingType}. Produk: ${productDesc}. Adegan: ${selectedSceneCount}. Balas HANYA JSON: {"script": "narasi", "prompts": ["visual 1", "visual 2"]}`;
            ...
            const plan = extractDataSafely(dataPlan);

            if (!plan || !plan.prompts) throw new Error("Gagal membaca naskah. Silakan coba lagi.");
            ...
            for (let i = 0; i < plan.prompts.length; i++) {
                ...
                const imgRes = await fetch(`${baseUrl}?key=${currentKey}`, {
                    method: 'POST',
                    body: JSON.stringify({ 
                        contents: [{ parts: [{ text: `${plan.prompts[i]}, high quality, 9:16 vertical` }] }],
                ...
                if (imgPart) {
                    ...
                    images.push("https://via.placeholder.com/1080x1920?text=Sensor+Aktif");
                }
        ...
        } catch (e) { alert("Sistem Sibuk: " + e.message); }
    ...
    return (
        <div className="min-h-screen bg-[#f3f4f6] text-gray-900 p-4 md:p-8 font-sans italic">
            <header className="max-w-7xl mx-auto flex justify-between items-center mb-8 border-b border-gray-200 pb-6">
                ...
                <button onClick={() => setShowApiModal(true)} className="p-2 bg-white border border-gray-300 rounded-full text-amber-500 shadow-sm"><Key size={20}/></button>
            </header>
            ...
            <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6 overflow-y-auto max-h-[85vh] pr-2 custom-scrollbar">
                    <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-6">
                            {['story-telling', 'promosi-produk'].map(style => (
                                <button key={style} onClick={() => setSelectedStyle(style)} className={`p-4 rounded-xl border-2 transition-all text-sm font-bold capitalize ${selectedStyle === style ? 'border-purple-600 bg-purple-50 text-purple-700 shadow-sm' : 'border-gray-50 bg-white'}`}>{style.replace('-', ' ')}</button>
                            ))}
                        </div>
                    ...
                    <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        ...
                        <div className="space-y-6">
                            <textarea value={productDesc} onChange={(e) => setProductDesc(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-xs h-24 outline-none focus:ring-2 focus:ring-purple-500" placeholder="Detail produk..."/>
                            ...
                            <div className="p-3 bg-purple-50 border border-purple-200 rounded-xl flex items-center justify-between">
                                ...
                                <span className="text-[9px] text-purple-500 font-bold uppercase tracking-tighter">TikTok/Reels</span>
                            </div>

                            <select value={sellingType} onChange={(e) => setSellingType(e.target.value)} className="w-full bg-white border border-gray-300 rounded-xl p-3 text-xs outline-none">
                        ...
                        <button onClick={handleGenerate} disabled={loading} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-4 rounded-xl mt-8 shadow-lg flex items-center justify-center gap-2">
                ...
                <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-200 h-[85vh] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
                        {!results ? (
                            <div className="h-full flex flex-col items-center justify-center opacity-20 text-center text-sm italic"><ImageIcon size={64} className="mb-4 text-purple-600 animate-pulse"/> Visual akan muncul di sini.</div>
                        ) : (
                            results.images.map((img, idx) => (
                                <div key={idx} className="relative group mb-4">
                                    ...
                                    <a href={img} download={`scene-${idx+1}.png`} className="absolute bottom-3 right-3 p-2 bg-white/90 rounded-full text-purple-600 shadow-md opacity-0 group-hover:opacity-100 transition-all"><Download size={14}/></a>
                                </div>
                            ))
                        )}
                    </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-200 h-[85vh] flex flex-col">
                    ...
                    {!results ? (
                        ...
                        <div className="space-y-4 h-full flex flex-col">
                            ...
                            <button onClick={() => navigator.clipboard.writeText(results.script)} className="w-full bg-gray-100 py-3 rounded-xl text-[10px] font-bold flex items-center justify-center gap-2 hover:bg-gray-200 transition-all shadow-sm"><Copy size={12}/> COPY NASKAH</button>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 relative shadow-2xl">
                        ...
                        <div className="space-y-3">
                            {apiKeys.map((key, idx) => (
                                <input key={idx} type="password" value={key} onChange={e => { const nk = [...apiKeys]; nk[idx] = e.target.value; setApiKeys(nk); }} placeholder={`Gemini Key #${idx + 1}`} className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-xs focus:ring-2 focus:ring-amber-500 outline-none shadow-sm" />
                            ))}
                        </div>
                        <button onClick={() => setShowApiModal(false)} className="w-full bg-gray-900 text-white font-bold py-4 rounded-2xl mt-6 shadow-xl transition-all uppercase tracking-widest">SIMPAN</button>
                    </div>
                </div>
            )}
            ...
            {loading && (
                <div className="fixed inset-0 bg-white/90 backdrop-blur-xl z-[100] flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <p className="text-lg font-bold text-gray-900 uppercase tracking-tighter">{loadingMsg}</p>
                    </div>
                </div>
            )}
        </div>
