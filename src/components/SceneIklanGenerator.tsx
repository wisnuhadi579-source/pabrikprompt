    ...
    return (
        <div className="min-h-screen bg-[#f3f4f6] text-gray-900 p-4 md:p-8 font-sans italic">
            <header className="max-w-7xl mx-auto flex justify-between items-center mb-8 border-b border-gray-200 pb-6">
                ...
                <button onClick={() => setShowApiModal(true)} className="p-2 bg-white border border-gray-300 rounded-full text-amber-500 shadow-sm hover:scale-110 transition-all shadow-amber-100"><Key size={20}/></button>
            </header>
            ...
            <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6 overflow-y-auto max-h-[85vh] pr-2 custom-scrollbar">
                    <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-6">
                            {['story-telling', 'promosi-produk'].map(style => (
                                <button key={style} onClick={() => setSelectedStyle(style)} className={`p-4 rounded-xl border-2 transition-all text-sm font-bold capitalize ${selectedStyle === style ? 'border-purple-600 bg-purple-50 text-purple-700 shadow-sm' : 'border-gray-50 bg-white hover:border-purple-200'}`}>{style.replace('-', ' ')}</button>
                            ))}
                        </div>
                        <div className="flex gap-2">
                            {[5, 8, 10].map(count => (
                                <button key={count} onClick={() => setSelectedSceneCount(count)} className={`flex-1 py-3 rounded-lg border text-sm font-bold transition-all ${selectedSceneCount === count ? 'bg-purple-600 text-white border-purple-600 shadow-lg' : 'bg-gray-50 text-gray-600 border-gray-200'}`}>{count} Scene</button>
                            ))}
                        </div>
                    </section>
                    ...
                    <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        ...
                        <button onClick={handleGenerate} disabled={loading} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-4 rounded-xl mt-8 shadow-lg shadow-purple-100 transition-all active:scale-95 flex items-center justify-center gap-2">
